    ...
    async def call_api(
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                data = (
                    [self.request_json, [str(e) for e in self.result], {"metadata": self.metadata}]
                    if self.metadata
            ...
            data = (
                [self.request_json, response, {"metadata": self.metadata}]
                if self.metadata
...
--requests_filepath data/datasets/prompted/fact_decomposition_20240821_DEBUG.jsonl \
...
--max_attempts 5 \
--logging_level 20 \
--model_name shc-gpt-4o \
--api_key XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX \
--generation_config src/factehr/clients/generation_params.json
  
async parallel processing 4x speedup
- 100 documents, iid sample of lengths
- 4096 generation length
serial processing:   1643.31 seconds
parallel processing:  413.55 seconds
"""
